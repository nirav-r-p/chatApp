...
class ChatViewModel() :ViewModel(){
    ...
    fun addMessage(recId:String, message:String){
        ...
        myDBRef.child("user").child(senderId!!).child("chats").child(senderRoom!!).child("unReadMessage").setValue(0)
        myDBRef.child("user").child(senderId!!).child("chats").child(senderRoom!!).child("messages").push()
            .setValue(messageObject).addOnCompleteListener {
                myDBRef.child("user").child(senderId!!).child("chats").child(senderRoom!!).child("lastTime").setValue(currentTime)
                myDBRef.child("user").child(recId!!).child("chats").child(receiverRoom!!).child("messages").push()
                    .setValue(messageObject).addOnCompleteListener {
                        myDBRef.child("user").child(recId!!).child("chats").child(receiverRoom!!).child("unReadMessage").setValue(1)
                        myDBRef.child("user").child(recId!!).child("chats").child(receiverRoom!!).child("lastMessage").setValue(message)
                        myDBRef.child("user").child(senderId!!).child("chats").child(senderRoom!!).child("lastMessage").setValue(message)
                    }
            }
